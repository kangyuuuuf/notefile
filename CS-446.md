...
As far we know that given data points $x$, how can we model $p(x)$?
...
- Fit mean and variance of a distribution (e.g., Gaussian) 
- Fit parameters θ of a mixture distribution 
- Use a variational auto-encoder (ELBO) 
- Use generative adversarial network (JSD)

Assume we have the following model:
$$
p(x; \theta) = \frac{e^{-E(x;\ theta)}}{Z(\theta)}
$$
where $E(x; \theta)$: energy function, $Z(\theta):$ partition function

![image-20231124134002525](CS-446.assets/image-20231124134002525.png)

![image-20231124134412632](CS-446.assets/image-20231124134412632.png)

#### Contrastive Divergence

A learning algorithm that used to update the model by using the contrast between model distribution and data distribution.

**Intuition**: make training data more likely than typical sample from the model.

#### Stochastic gradient Langevin dyanmics (SGLD)

Similar to Stochastic gradient, using small batch or one data point to update the gradient. **Langevin dyanmics** add an additional noise (usually normal distribution). This will add the randomness of the model, making we can randomly search the right parameter during the training. 

![image-20231124140708721](CS-446.assets/image-20231124140708721.png)

For each step, draw a sample $x$ sample using Langevin dynamics, then do a gradient ascent/descent step.
...
Policy Gradient is a policy optimization method used in reinforcement learning. Unlike value function methods (such as Q-learning or DDPG), policy gradient methods directly learn the parameters of the policy function rather than indirectly learning the value function. These methods are suitable for problems in discrete or continuous action spaces, and are particularly effective when dealing with high-dimensional or continuous action spaces.

![image-20231124144606954](CS-446.assets/image-20231124144606954.png)
...
where $\tau$ is a state-action sequence

![image-20231124155233685](CS-446.assets/image-20231124155233685.png)

And this approxmiation is vaild even if $R$ is discontinuous

![image-20231124160357115](CS-446.assets/image-20231124160357115.png)

#### Baseline

When we have $R(\tau) > 0 $ for any $\tau$, it will have an issue: since we directly modify the policy and it is a possibility, if we do not have sample $a$ for example during training, it will still reduce even it has a large reward. 

![image-20231124160651436](CS-446.assets/image-20231124160651436.png)
...
![image-20231124161349988](CS-446.assets/image-20231124161349988.png)

#### Algorithm

![image-20231124161431612](CS-446.assets/image-20231124161431612.png)
...
![image-20231124164302266](CS-446.assets/image-20231124164302266.png)

Problems with classical recurrent neural nets: **Vanishing gradients** and **Long term dependency**

#### Long short term memory (LSTM)

It can be interpreted as a block in a neural net work
...
![image-20231124164930322](CS-446.assets/image-20231124164930322.png)

![image-20231124164724772](CS-446.assets/image-20231124164724772.png)
...
![image-20231124165011862](CS-446.assets/image-20231124165011862.png)

#### Backpropagation through time (BPTT)

It is an extension of the backpropagation algorithm on time series data. It expands the RNN through time, converts it into a feedforward neural network, and applies the standard backpropagation algorithm on this basis.
